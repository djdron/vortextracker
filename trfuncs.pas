...
type
  ...
  (Unknown, STCFile, ASCFile, ASC0File, STPFile, PSCFile, FLSFile, FTCFile,
   PT1File, PT2File, PT3File, SQTFile, GTRFile, FXMFile);
...
procedure VTM2TextFile(FN:string;VTM:PModule);
...
for i := 1 to 15 do
 ...
 if PT2.PT2_OrnamentPointers[i] = 0 then
  ...
  begin
   ...
   if (VTM.Ornaments[i].Length = 0) or (VTM.Ornaments[i].Length > 64) then
 ...
 begin
  if PT2.PT2_SamplePointers[i] = 0 then
   ...
   begin
    ...
    if (VTM.Samples[i].Length = 0) or (VTM.Samples[i].Length > 64) then
    ...
    for j := 0 to VTM.Samples[i].Length - 1 do
     begin
      ...
      VTM.Samples[i].Items[j].Ton_Accumulation := False;
      VTM.Samples[i].Items[j].Amplitude_Sliding := False;
      VTM.Samples[i].Items[j].Amplitude_Slide_Up := False;
      VTM.Samples[i].Items[j].Envelope_Enabled := True;
      VTM.Samples[i].Items[j].Envelope_or_Noise_Accumulation := False;
      VTM.Samples[i].Items[j].Amplitude :=
 ...
 begin
  ...
  if l > 0 then
   begin
    ...
    until (k = 64) or (n = nb) or (n < -$55) or (n > $55);
    if (k = 1) and (n = 0) then
     begin
      zo := l;
      orns[i] := -1;
 ...
 begin
  ...
  if l > 0 then
   begin
    ...
    if (ASC.Index[j - 3] and (64 + 32) = 32) and (k < 64) then
     begin
      VTM.Samples[l].Loop := k;
      Inc(k);
...
var
 ...
 PrevOrn,PrevVol,PrevVol1,InitVol:array[0..2] of shortint;
...
//       Break_Sample_Loop := True //not available in PT3
 ...
 begin
  Result := False;
  j := PSC.PSC_SamplesPointers[sn] + $4c;
  k := 0;
  repeat
       ...
       if EnvEn[k] then
        if (VTM.Patterns[j].Items[i].Channel[k].Envelope = 15) or
         ...
         begin
          ...
          VTM.Patterns[j].Items[i].Channel[k].Ornament := PrevOrn[k];
          EnvSet[k] := True
         end;
      for k := 2 downto 0 do
 ...
 begin
  ...
  if l > 0 then
   begin
    New(VTM.Samples[l]);
    VTM.Samples[l].Loop := 0;
    j := PSC.PSC_SamplesPointers[i] + $4c;
    k := 0;
    repeat
     ...
     if k = 64 then break
    until PSC.Index[j - 2] and (64 + 32) in [0,32,64];
    if (PSC.Index[j - 2] and (64 + 32) = 64) and (k < 64) then
     begin
      VTM.Samples[l].Loop := k;
      Inc(k);
      VTM.Samples[l].Items[k - 1] := EmptySampleTick
  ...
  if l > 0 then
   begin
    ...
    repeat
     ...
     until j >= len;
     j := jl;
    until (k = 64) or (n = nb) or (n < -$5F) or (n > $5F);
    if (k = 1) and (n = 0) then
     begin
      zo := l;
      orns[i] := -1;
