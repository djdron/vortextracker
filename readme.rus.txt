...
Страница поддержки: http://bulba.at.kz/
...
следующих типов:
 ...
 8) компилированные модули ASC Sound Master (*.asc);
 9) компилированные модули Sound Tracker и Super Sonic (*.stc);
 10) компилированные модули Sound Tracker Pro (*.stp);
 11) компилированные модули SQ-Tracker (*.sqt).
...
как Pusher, Quick Commander, BestView, ZXAmp и другие. Вы также можете
встроить звук в свои программы, воспользовавшись исходниками Ay_Emul, пакета
YM-Engine или SquareTone (http://bulba.at.kz/).
...
(в последних двух плеер идет в виде отдельного файла).
...
Pro Tracker v3.6x от Alone Coder (a.k.a. Dima Bystrov). В режиме совместимости
"ProTracker 3.5" Vortex Tracker II полностью совместим с любыми подверсиями Pro
Tracker v3.5x.
...
инерпритация специальной команды 3xxx). Поэтому для того, чтобы модуль
...
касается редактора Pro Tracker 3 на ZX Spectrum, сейчас Alone Coder выпустил Pro
Tracker v3.692, который практически полностью совместим с новыми возможностями
VT II.
...
1. Копирование/вставка для редактора сэмплов и орнаментов.
2. Операции отмены (UnDo, ReDo, etc). 
3. Транспозиция выделенной области.
4. Шаблон нотной строки.
5. Разворачивание накоплений в сэмплах (для Alco).
6. Перестановка двух нот в соседних каналах (swap) для любителей стерео-
эффектов с автоматической подстановкой параметров.
7. Прогрессии, чередования.
8. Экспорт в WAV-файл.
9. ... :(
...
4. Команда Save работает как Save As...
5. Для SQT-файлов используется табличка #1 со второй ноты, что приводит к потере
нот #5E и #5F оригинального модуля (они превращаются в B-8).
6. Некоторые настройки нельзя изменить во время проигрывания, или когда
устройство вывода звука занято (во время редактирования). Нажмите кнопку
'Stop playing/Free device'.
7. Некоторые настройки вступают в силу через промежуток времени, равный
суммарной длине буферов.
8. Некоторые настройки вступают в силу сразу, но с небольшим щелчком, связанным
с временной приостановкой проигрывания.
9. Крайне неудобный регулятор громкости.
10.Буфер обмена не работает с редакторами/тестами сэмплов и орнаментов.
...
3.3. Движение мышью с нажатой левой кнопкой.
...
3.3. Ctrl+T - вызов диалога Tracks Manager.

3.4. Ctrl+Alt+T - вызов диалога Global Transposition.
...
Pro Tracker 3.692 на ZX Spectrum опознает и загружает модули VT II
(допустим как новый заголовок VT II, так и расширение *.pt3). Единственное
ограничение - не более 46 паттернов при длине не паттернов не более 64.
...
Pro Tracker 3.6xx почти полностью совместим с режимом 'Vortex Tracker II', в то
время как совместимость в режиме "Pro Tracker 3.5" со спектрумовским редактором
соответсвующей версии полная.
...
Данный заголовок говорит только о том, что команда 3xxx должна
интерпретироваться по-старому (можно использовать плееры только от PT 3.5x,
но лучше - универсальный плеер, прилагаемый к VT II).
...
иначе треки или будут не ровные, или курсор будет перемещаться по колонкам трека
невпопад. В данной версии выбранный шрифт используется и для некоторых других
элементов управления.

Вкладка 'Windows'

Настройка визуального расположения каналов микросхемы при редактировании. Влияет
как на уже открытые модули, так и на открываемые.
...
модуль, загляните на соответствующую вкладку его окна.
 ...
 - Vortex Tracker II - новая интерпретация команды 3xxx. Влияет не только на
                       проигрывание, но и на импорт модулей ASC.
 - Pro Tracker 3.5 - старое поведение команды 3xxx. Также, модули ASC будут
                     импортироваться с возможностью проигрывания со старыми
                     плеерами PT3.
 - Try detect - позволяет редактору во время открытия модуля самому выбрать один
                из первых двух пунктов. При открытии PT3 VT ориентируется по
                заголовку по описанному выше правилу. При открытии PT2
                выбирается пункт 'Pro Tracker 3.5', в остальных случаях -
                'Vortex Tracker II'.
...
Для вызова этой функции выберите соответствующий пункт в меню Edit или нажмите
Ctrl+T. С её помощью можно скопировать любую область любого паттерна в любое
...
Для вызова этой функции выберите соответствующий пункт в меню Edit или нажмите
Ctrl+Alt+T. С её помощью можно транспонировать отдельные (или все) дорожки как
всего модуля, так и отдельного паттерна, что позволяет избежать многократного
использования Tracks manager. Настройка и использование аналогично функции
Tracks manager.
...
информации, включая неиспользуемые сэмплы, орнаменты и паттерны.

Меню Files->Exports->Save in SNDH (Atari ST)
--------------------------------------------

Cохранение в формате SNDH для проигрывания на Atari ST (или SNDH плеерах-
...
анализируется версия модуля. За основу взяты процедуры из Ay_Emul. Получаемые
SNDH PACKICE'ом не упакованы. Размер плеера примерно 10 кб, адрес загрузки
не фиксирован.
...
модуля: при необходимсоти установите галочку "Disable loop". В файлах SCL и TAP
...
использованию находятся в файле ZXPlayer.rus.txt. Исходный текст плеера
...
- Славе Калинину за распространение редактора среди бывших AY-музыкантов.
...
чем поддержано в PT 3.xx на ZX - звук исказится.
...
24 августа 2002 года - 4 января 2005 года
