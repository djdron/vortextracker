...
uses Windows, SysUtils, Classes, Graphics, Forms, Controls, Menus,
  ...
  ActnList, ToolWin, ImgList, AY, WaveOutAPI, trfuncs, grids;
...
uses ChildWin, About, options, TrkMng, GlbTrn, ExportZX;
...
ResetMutex := CreateMutex(nil, False, 'VTII_Reset');
...
if (PlayingWindow.Tracks.ShownPattern <> PlayingWindow.VTMP.Patterns[pat])
 ...
 begin
  PlayingWindow.PatNum := pat;
  PlayingWindow.UpDown1.Position := pat;
  PlayingWindow.Tracks.ShownPattern := PlayingWindow.VTMP.Patterns[pat];
  PlayingWindow.Tracks.ShownFrom := Msg.LParam;
  PlayingWindow.CalculatePos(Msg.LParam);
  if PlayingWindow.Tracks.Enabled then HideCaret(PlayingWindow.Tracks.Handle);
  PlayingWindow.Tracks.CursorY := PlayingWindow.Tracks.N1OfLines;
  PlayingWindow.Tracks.RemoveSelection(0,True);
  PlayingWindow.Tracks.RedrawTracks(0);
  if PlayingWindow.Tracks.Enabled then ShowCaret(PlayingWindow.Tracks.Handle)
 end
end;
...
Form1.ChFreq.ItemIndex := Ord(AY_Freq <> 1773400);
Saved_AY_Freq := AY_Freq;
Form1.IntSel.ItemIndex := Ord(Interrupt_Freq <> 50000);
...
DisableControls;
PlayingWindow.Tracks.RemoveSelection(0,False);
PlayingWindow.RerollToPos(PlayingWindow.PositionNumber);
StartWOThread
end;
...
Module_SetCurrentPattern(PlayingWindow.PatNum);
...
RestoreControls;
PlayingWindow.Tracks.RemoveSelection(0,True);
if (TMDIChild(ActiveMDIChild) = PlayingWindow) and
   (PlayingWindow.PageControl1.ActivePageIndex = 0) then
 PlayingWindow.Tracks.SetFocus
end;
...
if IsPlaying then PlayingWindow.StopAndRestart
end;
...
SetDialogZXAYExt;
SaveDialogZXAY.FileName := '';
if SaveDialogZXAY.Execute then
 begin
